<template>
    <div class="list-container w100">
        <div class="flex-side list">
            <h2>{{ title }}</h2>
            <router-link class="theme-button" :to="{name: 'SettingCreate'}">добавить еще</router-link>
        </div>
            ...
            <table>
                ...
                <tbody>
                    <tr v-for="(user, index) in data" :key="`user-list-of-${index}`">
                        ...
                        <td><router-link :to="{name: 'SettingEdit', params: {id: user.id}}">{{ user.name }}</router-link></td>
                        <td width="100">{{ formatDate(user.createdAt) }}</td>
                        <td width="100">
                            <div class="flex-side list right">
                                <button @click="$router.push({name: 'SettingEdit', params: {id: user.id}})" class="theme-button xs" type="button">редактировать</button>
