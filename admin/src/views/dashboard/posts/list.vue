<template>
    <div class="list-container w100">
        <div class="flex-side list">
            <h2>{{ title }}</h2>
            <router-link class="theme-button" :to="{name: 'PostCreate'}">добавить еще</router-link>
        </div>
            ...
            <table>
                ...
                <tbody>
                    <tr v-for="(user, index) in data" :key="`user-list-of-${index}`">
                        ...
                        <td><router-link :to="{name: 'PostEdit', params: {id: user.id}}">{{ user.name }}</router-link></td>
                        <td><a :href="'mailto:' + user.User.email">{{ user.User.name }}</a></td>
                        <td width="100">{{ formatDate(user.createdAt) }}</td>
                        <td width="100">
                            <div class="flex-side list right">
                                <button @click="$router.push({name: 'PostEdit', params: {id: user.id}})" class="theme-button xs" type="button">редактировать</button>
